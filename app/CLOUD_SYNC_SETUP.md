...
- **Signed in yesterday, asked to sign in again today**: Google access tokens last about an hour and refresh silently in the background (via FedCM) while your browser has an active Google session; if that session itself expired (e.g. you signed out of Google entirely, or it's been a long time), you'll need to click Sign in again. Your data is never lost either way — it's always in `localStorage` on this device regardless of sync state.
- **Silent refresh doesn't work / asks to sign in on every reload**: the silent path relies on FedCM (Chrome/Edge's browser-mediated account check), which needs a modern Chromium-based browser. Safari and some hardened/privacy-focused browsers don't support FedCM and always block the third-party check it replaces — on those, you'll need to click Sign in each session. This is a browser limitation, not an app bug; your data is unaffected either way.
- **A one-time "Choose an account" / "Continue as ___" browser chip appears**: that's FedCM's native UI, shown once per browser per site the first time silent sign-in is attempted. It's expected and is what makes later reloads silent — not a phishing prompt.
