...
- **Reloading the page shows the account picker again**: Google's client-side sign-in only ever offers a dialog for getting a new access token — there's no fully silent grant, by Google's own design. So the app caches the token itself (not just "was signed in") across reloads, and only asks again once that token actually expires — about once per hour of continued use, not on every reload. If you're still seeing it every single reload, check the browser console for errors (e.g. a blocked popup, or `localStorage` being unavailable) — otherwise this is expected once per ~hour.
- **True "never ask again" persistence isn't possible with this setup**: doing so would require a real refresh-token flow, which needs a backend server to hold Google's client secret during the token exchange — GitHub Pages (a static host) can't do that. What's here (an hourly reprompt at most) is the best available without adding server infrastructure.
