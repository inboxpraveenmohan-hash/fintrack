...
- **Import CSV/Excel**: column layout is `Date, Items, Income, Expense, Account, Category` (`Item`, singular, is also accepted) — compatible with a daily-tracker spreadsheet you may already keep. Dates in `DD/MM/YYYY` are recognized, and amounts can be plain numbers or currency-formatted (`₹3,683.00`, `₹254.87`) — symbols, thousands separators, and spaces are stripped automatically. Unlike the Portfolio page's import (which replaces your data), importing here **adds** the file's transactions to your existing log — nothing is removed — and any category/account name it doesn't recognize is created automatically (income vs. expense is inferred from which CSV column has the amount).
