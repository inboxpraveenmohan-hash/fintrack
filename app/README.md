...
- **Transfers** ("Manage Transfers" button): for moving money between two of your own accounts — not income or spending, so excluded from the stat cards and charts. E.g. paying a credit card bill is a transfer from SAVINGS to CC, which correctly reduces both the SAVINGS balance and the CC amount owed at once. Kept in its own modal (like Manage Categories) rather than the main dashboard, since it's usually an occasional entry, not a daily one; shows your full transfer history, not just the current month.
- **Month selector** (◀ / ▶ in the toolbar): the stat cards, budget table, both charts, and transaction log are all scoped to one month at a time; account balances are always all-time regardless of which month you're viewing.
- **Total Expenses stat card**: its own ⚙ button opens a picker to choose which categories count toward this one number — e.g. exclude a category you don't consider "real" spending, like a large one-off purchase. This only affects the stat card itself; that category's own spend still shows normally in Budget vs Actual and both charts.
