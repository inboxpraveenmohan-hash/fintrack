...
- **Budget vs Actual**: lists every category. Most track spending — set a recurring monthly budget (applies to every month until you change it) and a progress bar shows spend as a % of budget, turning red past 100%. A category can have budgeting turned off entirely via the "Budget?" checkbox in Manage Categories (off by default for the seeded income categories, since a spending target doesn't make sense for them) — those show their income total instead, with no budget field or bar. Budgeted categories are listed first, then the rest.
