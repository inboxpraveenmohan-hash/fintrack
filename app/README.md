...
- **Budget vs Actual**: lists every leaf category. Most track spending — set a recurring monthly budget (applies to every month until you change it) and a progress bar shows spend as a % of budget, turning red past 100%. A category can have budgeting turned off entirely via the "Budget?" checkbox in Manage Categories (off by default for the seeded income categories, since a spending target doesn't make sense for them) — those show their income total instead, with no budget field or bar. Either way, if the category also has activity in the *other* direction (e.g. a refund logged as "In" under an expense category), that's shown as a small "+₹x in/out" hint next to the main figure rather than being hidden. Budgeted categories are listed first, then the rest. A top-level group (e.g. "Expense") can also get its own budget row — check its own "Budget?" box in Manage Categories — in which case its actual figure rolls up every child category's activity, letting you set one overall budget for the whole group in addition to (not instead of) each child's own budget.
