...
- **Budget vs Actual**: lists every leaf category. "Actual" is the net of out minus in for that category (e.g. a rare refund logged as "In" under an expense category reduces its effective spend), always shown as a plain non-negative number. Most categories track spending — set a recurring monthly budget (applies to every month until you change it) and a progress bar shows spend as a % of budget, turning red past 100%; if "In" activity actually outweighs "Out" for the month (net negative), the bar shows in blue instead, since it's no longer really tracking overspend. A category can have budgeting turned off entirely via the "Budget?" checkbox in Manage Categories (off by default for the seeded income categories, since a spending target doesn't make sense for them, and unavailable for top-level groups) — those still show their actual total, just with no budget field or bar. Budgeted categories are listed first, then the rest.
